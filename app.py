...
firebase_admin.initialize_app(cred, {
    'databaseURL': 'https://yuvi-hosting-net-default-rtdb.firebaseio.com/'  # Apni URL yahan daalein
})
    ...
    if request.method == "POST":
        ...
        if l_type == "admin":
            ...
            if username and username not in db_data["users"]:
                ref.child("users").child(username).set(db_data["user_pw"])
                db_data = load_db()
...
# --- UPDATED RUN ROUTE FOR ANY PYTHON/JS FILE ---
@app.route("/run/<name>")
def run(name):
    user_name = session['username']
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        ...
        if os.path.exists(extract_dir):
            ...
            # 1. Pehle standard files check karein priority ke liye
            standard_files = ["main.py", "bot.py", "app.py", "index.js", "server.js"]
            main_file = next((f for f in standard_files if f in all_files), None)
            
            # 2. Agar standard file nahi mili, toh folder ki pehli .py ya .js file pick kar lein (e.g. sexy.py)
            if not main_file:
