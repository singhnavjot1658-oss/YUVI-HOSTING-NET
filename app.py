...
def stop_and_clean_user(user_name):
    ...
    for key, p in list(processes.items()):
        if key[0] == user_name:
            try:
                # Sabhi nested asynchronous event loops ya sub-processes ko completely stop karne ke liye force-kill
                if hasattr(os, 'killpg'):
                    os.killpg(os.getpgid(p.pid), signal.SIGKILL)
                else:
                    p.kill() # Standard platform independent bypass
            except:
                try:
                    p.terminate()
                except:
            ...
            # System tasks control cleanup commands
            if os.name == 'nt': # Windows environment check
...
def enforcement_loop():
    while True:
        ...
        time.sleep(5) # Direct engine checks targeted down to 5 seconds for critical auto-terminate
...
def get_alert():
    ...
    # SYSTEM ACCESS ENGINE CRITICAL PROTECTION: Real-time dynamic kick interceptor
    if not session.get('is_admin') and user_name in db["users"]:
        ...
        if u_info.get("plan_type") != "permanent" and u_info.get("status") == "expired":
            session.clear() # Clear state immediately
...
def bg_smart_scan_and_run(user_name, name, app_dir, extract_dir):
    ...
    # Execution setup with process groups for global kills
    for f in os.listdir(extract_dir):
        ...
        if os.path.isfile(file_path) and (f.endswith('.py') or f.endswith('.js')):
            ...
            if process_key not in processes or processes[process_key].poll() is not None:
                ...
                # Attaching preexec_fn for UNIX process isolation group
                kwargs = {}
