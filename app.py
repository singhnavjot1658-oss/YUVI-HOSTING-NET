...
import os, zipfile, subprocess, shutil, json, time, io, threading, signal, psutil
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .header { 
            ...
            margin-bottom: 25px; 
            padding: 18px 22px; 
            background: linear-gradient(135deg, rgba(12, 18, 35, 0.9), rgba(6, 9, 19, 0.95)); 
            border-radius: 15px; 
        ...
        /* USER DIRECTORY ITEM */
        .user-item { background: rgba(4, 7, 16, 0.7); border-radius: 10px; padding: 14px 16px; margin-bottom: 12px; border: 1px solid rgba(255,255,255,0.03); }
        ...
        .status-badge { padding: 3px 10px; border-radius: 12px; font-size: 9px; font-weight: bold; letter-spacing: 0.5px; text-transform: uppercase; }
        ...
        .metric-title { font-size: 10px; color: var(--neon-blue); font-family: 'Rajdhani', sans-serif; text-transform: uppercase; }
        .metric-value { font-size: 16px; font-weight: bold; color: #fff; }

        pre { background: #010204; color: #00ffff; padding: 12px; border-radius: 8px; font-size: 11px; overflow-x: auto; border: 1px solid rgba(0, 255, 255, 0.1); font-family: monospace; }
    </style>
    <script>
    ...
    <div class="container">
        ...
        <div class="section-box">
            <div class="section-header-title">
                <div><i class="fa-solid fa-server"></i> SECTION 1: SERVER HEALTH & EMERGENCY CONTROLS</div>
                <span class="section-tag">Diagnostics & Status</span>
            </div>
            <div class="grid">
                <div class="card-inner">
                    <p style="font-size:11px; color:var(--neon-blue); font-family:'Rajdhani'; margin-bottom:10px;"><i class="fa-solid fa-microchip"></i> LIVE RESOURCE MONITOR</p>
                    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px;">
                        <div class="metric-box">
                            <span class="metric-title">CPU Usage</span>
                            <span class="metric-value">{{ sys_info.cpu }}%</span>
                        </div>
                        <div class="metric-box">
                            <span class="metric-title">RAM Usage</span>
                            <span class="metric-value">{{ sys_info.ram }}%</span>
                        </div>
                        <div class="metric-box">
                            <span class="metric-title">Active Bots</span>
                            <span class="metric-value">{{ sys_info.active_processes }}</span>
                        </div>
                    </div>
                </div>

                <div class="card-inner">
                    <p style="font-size:11px; color:var(--neon-blue); font-family:'Rajdhani'; margin-bottom:10px;"><i class="fa-solid fa-bolt"></i> EMERGENCY CONTROLS</p>
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <form action="/admin/toggle_maintenance" method="post" style="margin:0;">
                            <button type="submit" class="btn {% if maintenance_mode %}btn-warning{% else %}btn-primary{% endif %}" style="width:100%;">
                                <i class="fa-solid fa-screwdriver-wrench"></i> 
                                Maintenance Mode: {{ "ENABLED (ON)" if maintenance_mode else "DISABLED (OFF)" }}
                            </button>
                        </form>
                        <a href="/admin/kill_all_processes" class="btn btn-danger" onclick="return confirm('Kill ALL running user scripts/bots immediately?')">
                            <i class="fa-solid fa-skull"></i> FORCE KILL ALL USER PROCESSES
                        </a>
                    </div>
                </div>
            </div>
        ...
        <div class="section-box">
            <div class="section-header-title">
                <div><i class="fa-solid fa-user-plus"></i> SECTION 2: ADD NEW SUBSCRIBER</div>
                <span class="section-tag">Account Creation</span>
            </div>
            <form action="/admin/create_user" method="post" class="input-group">
                ...
                <button type="submit" class="btn btn-primary" style="width:100%;"><i class="fa-solid fa-plus-circle"></i> Create Account</button>
            </form>
        </div>

        <div class="section-box">
            <div class="section-header-title">
                <div><i class="fa-solid fa-bullhorn"></i> SECTION 3: BROADCAST STATION</div>
                <span class="section-tag">Global Alerts</span>
            </div>
            <form action="/admin/broadcast" method="post" class="input-group">
                ...
                <button type="submit" class="btn btn-primary" style="width:100%;"><i class="fa-solid fa-paper-plane"></i> Send Broadcast Alert</button>
            </form>
        </div>

        <div class="section-box">
            <div class="section-header-title">
                <div><i class="fa-solid fa-shield-halved"></i> SECTION 4: SECURITY & AUTHENTICATION</div>
                <span class="section-tag">System Credentials</span>
            </div>
            <div class="grid">
                <div class="card-inner">
                    <p style="font-size:11px; color:var(--neon-blue); font-family:'Rajdhani'; margin-bottom:10px;"><i class="fa-solid fa-lock"></i> UPDATE ADMIN CREDS</p>
                    <form action="/admin/update_creds" method="post" class="input-group">
                        ...
                        <button type="submit" class="btn btn-primary"><i class="fa-solid fa-save"></i> Save Admin Info</button>
                    </form>
                </div>

                <div class="card-inner">
                    <p style="font-size:11px; color:var(--neon-blue); font-family:'Rajdhani'; margin-bottom:10px;"><i class="fa-solid fa-robot"></i> TELEGRAM BOT SECRET API KEY</p>
                    <form action="/admin/update_api_key" method="post" class="input-group">
                        <label style="font-size:10px; color: var(--neon-blue); font-family:'Rajdhani';">SECRET API KEY FOR BOT AUTH:</label>
        ...
        <div class="section-box">
            <div class="section-header-title">
                <div><i class="fa-solid fa-code"></i> SECTION 5: BOT INTEGRATION API</div>
                <span class="section-tag">Developer Docs</span>
            </div>
            <p style="font-size: 11px; margin-bottom: 10px; opacity: 0.8; font-family: sans-serif;">Use this endpoint in your Python scripts to register users remotely:</p>
            <pre>
...
BODY:
{
    ...
    "expiry": "2026-12-31T23:59"
        ...
        <div class="section-box">
            <div class="section-header-title">
                <div><i class="fa-solid fa-users"></i> SECTION 6: SUBSCRIBER DIRECTORY</div>
                <span class="section-tag">Active Database</span>
            </div>
            {% if users %}
