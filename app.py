    ...
    while True:
        try:
            ...
            for username, info in list(db["users"].items()):
                # Permanent system integration check
...
# --- UI LOGINS (Bada Box Mota/Chouda Kiya Gaya Hai) ---
LOGIN_HTML = '''
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .login-card { position: relative; z-index: 10; background: var(--glass); padding: 40px 35px; border-radius: 25px; width: 100%; max-width: 420px; text-align: center; border: 1px solid rgba(0, 255, 255, 0.15); backdrop-filter: blur(20px); box-shadow: 0 25px 45px rgba(0,0,0,0.5); transition: 0.4s; }
    ...
    <div class="grid">
        <div class="card">
            ...
            <form action="/admin/create_user" method="post" class="input-group">
                ...
                <select name="plan_type" id="plan_type" onchange="if(this.value=='permanent'){document.getElementById('exp_input').style.display='none'; document.getElementById('exp_input').removeAttribute('required');}else{document.getElementById('exp_input').style.display='block'; document.getElementById('exp_input').setAttribute('required','required');}">
...
def index():
    ...
    # Extracting current dynamic expiry value to pass inside template execution context
...
def create_user():
    ...
    expiry = request.form.get("expiry") if plan_type == "time_bound" else "2099-12-31T23:59"
