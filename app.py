...
# FIXED: Standard Index template containing dynamic CSS Custom properties (--theme-color) to save custom colors from resetting.
INDEX_HTML = '''
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dashboard | YUVI HOSTING</title>
    <link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@500;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        :root {
            --theme-color: {{ user_color }};
            --glow-size: {{ user_size }}px;
            --speed: {{ user_speed }}s;
            --bg-dark: #030508;
        }
        body { background: var(--bg-dark); color: white; font-family: 'Orbitron', sans-serif; padding: 20px; }
        .control-panel { border: 1px solid var(--theme-color); padding: 20px; border-radius: 15px; box-shadow: 0 0 var(--glow-size) var(--theme-color); margin-bottom: 20px; }
        .color-picker-box { display: flex; align-items: center; gap: 15px; margin-top: 15px; background: rgba(255,255,255,0.02); padding: 10px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.05); }
        .btn { padding: 8px 15px; background: var(--theme-color); border: none; border-radius: 5px; color: black; font-weight: bold; cursor: pointer; text-transform: uppercase; }
        .app-card { border: 1px solid rgba(255,255,255,0.1); padding: 15px; border-radius: 10px; margin-top: 10px; display: flex; justify-content: space-between; align-items: center; }
...
<body>
    <div class="control-panel">
        ...
        <p>Plan Expiry: <span style="color: var(--theme-color);">{{ plan_expiry }}</span></p>
        
        <div class="color-picker-box">
            <label><i class="fa-solid fa-palette"></i> Choose Theme Color: </label>
            <input type="color" id="themeColor" value="{{ user_color }}">
            <button class="btn" onclick="saveThemeSettings()">Save Color Style</button>
        </div>
        <br>
        <a href="/logout" style="color: #ff4757; text-decoration: none;"><i class="fa-solid fa-power-off"></i> Logout Dashboard</a>
    </div>

    <h3>Your Deployments</h3>
    <form action="/upload" method="post" enctype="multipart/form-data" style="margin-bottom: 20px;">
        <input type="file" name="file" accept=".zip" required>
        <button type="submit" class="btn">Upload Project Zip</button>
    </form>
    ...
    <div class="app-card">
        <div>
            ...
            <span class="{% if app.running %}status-running{% else %}status-offline{% endif %}">
                {% if app.running %}RUNNING{% else %}OFFLINE{% endif %}
            </span>
        </div>
        <div>
            {% if not app.running %}
                <a href="/run/{{ app.name }}" class="btn" style="background: #00ff00; color: black; padding: 4px 10px; text-decoration: none;">Run</a>
            {% else %}
                <a href="/stop/{{ app.name }}" class="btn" style="background: #ff0000; color: white; padding: 4px 10px; text-decoration: none;">Stop</a>
            {% endif %}
            <a href="/delete/{{ app.name }}" style="color: #ff4757; margin-left: 15px; text-decoration: none;" onclick="return confirm('Delete deployment?')"><i class="fa-solid fa-trash"></i></a>
        </div>
    </div>
    {% endfor %}
    ...
    <script>
        function saveThemeSettings() {
            const chosenColor = document.getElementById('themeColor').value;
            fetch('/save_theme', {
                method: 'POST',
            ...
            .then(data => {
                if(data.status === 'success') {
                    alert('Theme setup successfully updated & synchronized!');
...
def index():
    ...
    # Dynamic dynamic rendering standard page index template string string fallback
