...
DB_FILE = "database.json"  # Instantly save karne ke liye local DB file
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

# PROCESSES TRACKING FIX: Ab hum key me file_name bhi track karenge -> (user_name, project_name, file_name)
processes = {}
...
if not firebase_admin._apps:
    ...
    if firebase_creds_json:
        try:
            ...
            print(f"Firebase Storage successfully initialized with bucket: {bucket_name}")
        except Exception as e:
            print(f"Firebase Storage Initialization Error: {e}")
    else:
        print("WARNING: FIREBASE_CREDS Environment Variable nahi mila! Storage backup kaam nahi karega.")

# --- LOCAL DB FUNCTIONS (FIXED INSTANT PASSWORD & THEME SAVE) ---
def load_db():
...
<body>
    ...
    <div class="header"><h2><i class="fa-solid fa-shield-halved"></i> JUBARAJ ROOT</h2><a href="/logout" class="btn btn-logout"><i class="fa-solid fa-power-off"></i></a></div>
...
def index():
    ...
    for name in os.listdir(user_dir):
        if os.path.isdir(os.path.join(user_dir, name)):
            # Check if ANY file inside this project is running
...
# --- NEW ROUTE: SAVE PER-USER THEME OPTIONS INDEPENDENTLY ---
...
def change_pw():
    ...
    if u_name in db["users"]:
        ...
        save_db(db)  # Local JSON par bina delay instantly save ho jayega!
    return redirect(url_for("admin_panel"))
...
# --- ANTI-LAG BACKGROUND SYSTEM WORKER ---
def bg_smart_scan_and_run(user_name, name, app_dir, extract_dir):
    ...
    with open(log_path, "a") as log_file:
        # 1. SMART CHECK: Agar requirements.txt nahi h, to automatically generate karo bina lag ke
        if not os.path.exists(req_file_path):
            log_file.write(f"\n--- [{time.strftime('%Y-%m-%d %H:%M:%S')}] Smart Scan: requirements.txt missing. Scanning imports... ---\n")
            log_file.flush()
            try:
                subprocess.run(
                    ["pipreqs", extract_dir, "--force"],
                    stdout=log_file,
                    stderr=log_file,
                    text=True,
                    check=True
                )
                log_file.write("--- Smart Scan Complete: requirements.txt auto-generated! ---\n")
            except Exception as e:
                log_file.write(f"--- Smart Scan Warning: {e} (Proceeding to run) ---\n")

        # 2. AUTO INSTALLATION: Generated ya user ki di hui requirements install karo
        if os.path.exists(req_file_path) and os.path.getsize(req_file_path) > 0:
            log_file.write("--- Syncing Modules / Installing Dependencies... ---\n")
            log_file.flush()
            try:
                subprocess.run(
                ...
                log_file.write("--- Dependencies Verified & Active! ---\n\n")
            except subprocess.CalledProcessError as e:
                log_file.write(f"--- Process Warning during module setup: {e} ---\n\n")
        else:
            log_file.write("--- No external modules detected. Starting process... ---\n\n")
        log_file.flush()
...
# --- UPDATED DYNAMIC ROUTE WITHOUT WEB INTERRUPTION ---
@app.route("/run/<name>")
def run(name):
    user_name = session['username']
    ...
    if os.path.exists(extract_dir):
        # Yahan process heavy hai isliye threading backend trigger kar di bina app slow kiye
        threading.Thread(target=bg_smart_scan_and_run, args=(user_name, name, app_dir, extract_dir)).start()
        
    return redirect(url_for("index"))

# --- FIX: GET STATUS FOR EVERY ALIVE SCRIPT ---
...
def get_log(name):
    ...
    # Check if at least one script process is alive
    is_running = False
    db = load_db()
    oldest_time = 0
    ...
    for key, p in list(processes.items()):
        if key[0] == user_name and key[1] == name and p.poll() is None:
            ...
            if oldest_time == 0 or db["start_times"].get(t_key, 0) < oldest_time:
                oldest_time = db["start_times"].get(t_key, 0)
...
# --- FIX: STOP ALL BG ATTACHED SCRIPT PROCESSES ---
@app.route("/stop/<name>")
def stop(name):
    user_name = session.get('username')
    db = load_db()
    ...
    if file and file.filename.endswith(".zip"):
        ...
        if firebase_admin._apps:
            try:
                ...
                print(f"Backup saved to Firebase Storage: backups/{user_name}/{file.filename}")
            except Exception as e:
                print(f"Firebase Storage Backup Error: {e}")
...
def delete(name):
    ...
    if firebase_admin._apps:
        try:
            ...
            if blob.exists():
                blob.delete()
                print(f"Backup deleted from Firebase Storage: backups/{user_name}/{name}.zip")
        except Exception as e:
