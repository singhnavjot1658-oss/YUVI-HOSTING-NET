...
# Firebase Modules Import Kiye
import firebase_admin
from firebase_admin import credentials
from firebase_admin import db as firebase_db

app = Flask(__name__)
app.secret_key = "YUVI_HOSTING_X"

# Render par temporary data ke liye sahi folder path
...
try:
    ...
    firebase_admin.initialize_app(cred, {
        'databaseURL': 'https://yuvi-hosting-default-rtdb.firebaseio.com/'
    })
    print(">>> Firebase Connected Successfully! <<<")
except Exception as e:
    print(f">>> Firebase Init Error: {e} <<<")

# --- FIREBASE DB FUNCTIONS (LOAD & SAVE ALTERNATIVE) ---
def load_db():
    try:
        ref = firebase_db.reference('/')
        data = ref.get()
        if not data:
            default = {"user_pw": "codex123", "users": {}, "start_times": {}, "user_themes": {}}
            ref.set(default)
            return default
        
        if "users" not in data or data["users"] is None: data["users"] = {}
        if "user_pw" not in data: data["user_pw"] = "codex123"
        if "start_times" not in data or data["start_times"] is None: data["start_times"] = {}
        if "user_themes" not in data or data["user_themes"] is None: data["user_themes"] = {}
        return data
    except Exception as e:
        print(f"Firebase Load Error: {e}")
        return {"user_pw": "codex123", "users": {}, "start_times": {}, "user_themes": {}}
...
<head>
    ...
    <style>
        ...
        .lock-container { width: 80px; height: 80px; background: rgba(0, 255, 255, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 hover; margin: 0 auto 20px; border: 2px solid var(--primary); box-shadow: 0 0 20px var(--primary); animation: pulse 2s infinite; }
...
<body>
    ...
    <div class="login-card">
        ...
        <div class="get-pw"><a href="https://t.me/ItsYuvi_LEGACY" target="_blank"><i class="fa-brands fa-telegram"></i> FORGOT PASSWORD?</a></div>
    </div>
...
<body>
    ...
    <div class="header"><h2><i class="fa-solid fa-shield-halved"></i> ROOT - JUBARAJ</h2><a href="/logout" class="btn btn-logout"><i class="fa-solid fa-power-off"></i></a></div>
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .menu-toggle { position: fixed; top: 70px; right: 15px; width: 45px; height: 45px; background: rgba(0, 255, 255, 0.1); border: 1px solid var(--accent); border-radius: 50%; display: flex; justify-content: center; align-items: center; color: var(--accent); font-size: 18px; cursor: pointer; z-index: 1000; backdrop-filter: blur(10px); box-shadow: 0 0 15px rgba(0, 255, 255, 0.3); transition: 0.4s ease; }
        .cyber-panel { position: fixed; top: 125px; right: -300px; width: 260px; background: rgba(10, 15, 20, 0.95); padding: 25px; border-radius: 20px; backdrop-filter: blur(20px); border: 1px solid var(--accent); z-index: 999; transition: 0.5s cubic-bezier(0.68, -0.55, 0.27, 1.55); display: flex; flex-direction: column; gap: 20px; }
        ...
        .setting-label { font-size: 10px; color: var(--accent); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px; display: block; }
        ...
        .upload-card { background: var(--box-bg); padding: 25px; border-radius: 20px; border: 2px dashed var(--accent); backdrop-filter: blur(12px); margin-bottom: 20px; transition: 0.3s; }
        #file-details { margin-top: 15px; padding: 12px; background: rgba(0, 255, 255, 0.05); border: 1px solid var(--accent); border-radius: 12px; display: none; animation: fadeIn 0.4s ease; }
...
<div class="cyber-panel" id="sidePanel">
    ...
    <div>
        ...
        <div class="color-dots-container">
            <div class="color-dot" data-color="#00ffff" style="background: #00ffff; color: #00ffff;" onclick="changeHue('#00ffff', this)"></div>
            <div class="color-dot" data-color="#ff004c" style="background: #ff004c; color: #ff004c;" onclick="changeHue('#ff004c', this)"></div>
            <div class="color-dot" data-color="#00ff00" style="background: #00ff00; color: #00ff00;" onclick="changeHue('#00ff00', this)"></div>
            <div class="color-dot" data-color="#ff00ff" style="background: #ff00ff; color: #ff00ff;" onclick="changeHue('#ff00ff', this)"></div>
            <div class="color-dot" data-color="#ffff00" style="background: #ffff00; color: #ffff00;" onclick="changeHue('#ffff00', this)"></div>
            <div class="color-dot" data-color="#ff8c00" style="background: #ff8c00; color: #ff8c00;" onclick="changeHue('#ff8c00', this)"></div>
            <div class="color-dot" data-color="#1e90ff" style="background: #1e90ff; color: #1e90ff;" onclick="changeHue('#1e90ff', this)"></div>
            <div class="color-dot" data-color="#ffffff" style="background: #ffffff; color: #ffffff;" onclick="changeHue('#ffffff', this)"></div>
            <div class="color-dot" data-color="#a020f0" style="background: #a020f0; color: #a020f0;" onclick="changeHue('#a020f0', this)"></div>
            <div class="color-dot" data-color="#00ffcc" style="background: #00ffcc; color: #00ffcc;" onclick="changeHue('#00ffcc', this)"></div>
        </div>
    </div>
    <div><span class="setting-label"><i class="fa-solid fa-expand"></i> Node Size</span><input type="range" id="pSize" min="10" max="80" value="38" oninput="updateSize(this.value)"></div>
    <div><span class="setting-label"><i class="fa-solid fa-gauge-high"></i> Velocity</span><input type="range" id="pSpeed" min="1" max="15" value="4" oninput="updateSpeed(this.value)"></div>
</div>
<div class="user-bar">
...
<div class="container">
    ...
    <div class="branding">
        ...
        <p style="font-size: 11px; color: #8b949e; letter-spacing: 3px;">LOVE YOU ALL. SUPPORT KARO</p>
    </div>
</div>
<div id="fmModal">
...
<div class="social-bar">
    ...
    <a href="https://youtube.com/@alpharush-tv?si=pymIHcn8UPXFS11e" target="_blank" class="social-icon"><i class="fa-brands fa-youtube"></i></a>
...
<script>
    ...
    function refreshFiles() { fetch('/list_files/' + activeProj).then(res => res.json()).then(data => { let html = ""; data.files.forEach(f => { html += `<div class="file-item"><span onclick="loadFile('${f}')" style="cursor:pointer; color:var(--accent); font-size:13px;">📄 ${f}</span><i class="fa-solid fa-trash-can" onclick="deleteFile('${f}')" style="color:#ff4d4d; cursor:pointer;"></i></div>`; }); document.getElementById('fileList').innerHTML = html || "Empty Project"; }); }
    function loadFile(f) { activeFile = f; fetch('/read_file', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({project: activeProj, filename: f}) }).then(res => res.json()).then(data => { editor.setValue(data.content, -1); document.getElementById('saveBtn').style.display = 'block'; if(f.endsWith('.js')) editor.session.setMode("ace/mode/javascript"); else if(f.endsWith('.py')) editor.session.setMode("ace/mode/python"); else editor.session.setMode("ace/mode/text"); }); }
    function saveFile() { fetch('/save_file', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({project: activeProj, filename: activeFile, content: editor.getValue()}) }).then(res => res.json()).then(() => { Swal.fire({ icon: 'success', title: 'Saved!', background: '#0a0a0a', color: '#fff', timer: 1000, showConfirmButton: false }); }); }
    function deleteFile(f) { Swal.fire({ title: 'মুছে ফেলবেন?', text: `${f} স্থায়ীভাবে ডিলিট হবে!`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ff4d4d', background: '#0a0a0a', color: '#fff' }).then(res => { if(res.isConfirmed) { fetch('/delete_file', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({project: activeProj, filename: f}) }).then(() => refreshFiles()); } }); }
    document.getElementById('fileInput').addEventListener('change', function(e) { const file = e.target.files[0]; const detailBox = document.getElementById('file-details'); if (file) { document.getElementById('file-name').innerText = file.name; let size = file.size / 1024; document.getElementById('file-size').innerText = size > 1024 ? (size / 1024).toFixed(2) + " MB" : size.toFixed(2) + " KB"; detailBox.style.display = 'block'; } else { detailBox.style.display = 'none'; } });
    ...
    function changeHue(newColor, element) { 
        ...
        // Backend pe user ka color permanent save karne ke liye fetch request
        fetch('/save_theme', {
            method: 'POST',
    ...
    // Page load hote hi user ka purana saved color dhoondhna
    function loadUserTheme() {
        fetch('/get_theme')
        .then(res => res.json())
        .then(data => {
            if(data.color) {
                pConfig.color = data.color;
    ...
    function updateSystem() {
        {% for app in apps %}
            fetch('/get_log/{{ app.name }}').then(res => res.json()).then(data => { const badge = document.getElementById('status-{{ app.name }}'); const logBox = document.getElementById('log-{{ app.name }}'); if (data.log) logBox.innerText = data.log; if (data.status === "RUNNING") { badge.innerText = "● RUNNING"; badge.className = "status-badge status-running"; if (data.start_time > 0) startTimes['{{ app.name }}'] = data.start_time; } else { badge.innerText = "● OFFLINE"; badge.className = "status-badge status-offline"; delete startTimes['{{ app.name }}']; } });
        {% endfor %}
    }
    function runTimers() { Object.keys(startTimes).forEach(name => { const diff = Date.now() - startTimes[name]; if (diff > 0) { const d = Math.floor(diff / 86400000); const h = Math.floor((diff % 86400000) / 3600000); const m = Math.floor((diff % 3600000) / 60000); const s = Math.floor((diff % 60000) / 1000); document.getElementById(`timer-${name}`).innerText = `${String(d).padStart(2,'0')}d ${String(h).padStart(2,'0')}h ${String(m).padStart(2,'0')}m ${String(s).padStart(2,'0')}s`; } }); }
    setInterval(updateSystem, 3000); setInterval(runTimers, 1000); 
    
    // Default call ko hata kar custom load script lagayi
    loadUserTheme();
    
    function confirmDelete(name, url) { Swal.fire({ title: 'মুছে ফেলবেন?', text: `${name} মুছে डाला जाएगा!`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ff4d4d', background: '#0a0a0a', color: '#fff' }).then(res => { if(res.isConfirmed) window.location.href = url; }); }
</script>
</body>
</html>
'''

# --- NEW API FOR THEME SAVING & LOADING ---
...
def save_theme():
    ...
    db = load_db()
    db["user_themes"][user_name] = color
    save_db(db)
    return jsonify({"status": "success"})

@app.route("/get_theme")
def get_theme():
    if 'username' not in session: return jsonify({"color": "#00ffff"})
    user_name = session['username']
    db = load_db()
    color = db["user_themes"].get(user_name, "#00ffff")
...
def login():
    if request.method == "POST":
        ...
        db = load_db()
        if l_type == "admin":
            if username == "JUBARAJ" and pw == ADMIN_PASS:
                session['is_admin'] = True
                session['username'] = "JUBARAJ"
...
@app.route("/admin")
def admin_panel():
    if not session.get('is_admin') or session.get('username') != "JUBARAJ": 
...
def global_pw():
    if not session.get('is_admin') or session.get('username') != "JUBARAJ": return redirect(url_for("login"))
    db = load_db()
...
def run(name):
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        main_file = None
        for f in ["main.py", "bot.py", "app.py", "index.js", "server.js"]:
            if os.path.exists(os.path.join(extract_dir, f)):
                main_file = f
                break
        
        if not main_file and os.path.exists(extract_dir):
            for root, _, files_list in os.walk(extract_dir):
                for f in files_list:
                    if f.endswith('.py') or f.endswith('.js'):
                        main_file = os.path.relpath(os.path.join(root, f), extract_dir)
                        break
                if main_file: break
...
def get_log(name):
    ...
    if not user_name: return jsonify({"log": "Session Expired", "status": "OFFLINE", "start_time": 0})
    app_dir = os.path.join(UPLOAD_FOLDER, user_name, name)
    log_path = os.path.join(app_dir, "logs.txt")
    log_content = ""
...
def download(name):
    ...
    if not os.path.exists(app_dir): return "Project not found", 404
