...
def stop_and_clean_user(user_name):
    ...
    # === FIX: Mark user as expired in the database itself ===
    # This ensures the status is always persisted even if the caller's
    # subsequent save_db() encounters a race condition or failure
            ...
            for username, info in list(db["users"].items()):
                ...
                if expiry_str:
                    ...
                    if now > expiry_ts and info.get("status") != "expired":
                        # stop_and_clean_user now sets status="expired" and saves independently
                        stop_and_clean_user(username)
                        # === FIX: Reload fresh db to prevent stale data overwrite ===
                        db = load_db()
                        changed = True
            if changed:
                save_db(db)
...
def login_as(username):
    if not session.get('is_admin'): return redirect(url_for("login"))
    session['username'], session['is_admin'] = False, username
    return redirect(url_for("index"))
