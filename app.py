...
# ⚠️ Dhyaan rakhein: serviceAccountKey.json file aapke main folder me honi chahiye!
try:
...
def load_db():
    try:
        ...
        data = ref.get()
        if not data:
            # Agar Firebase khali hai toh default data setup karein
            default = {"user_pw": "codex123", "users": {}, "start_times": {}}
            ref.set(default)
            return default
        
        # Tasalli ke liye keys check kar rahe hain
...
<div class="social-bar">
    <a href="https://t.me/lambhaicodex" target="_blank" class="social-icon"><i class="fa-brands fa-telegram"></i></a>
    <a href="https://www.youtube.com/@LamBhaiHK123" target="_blank" class="social-icon"><i class="fa-brands fa-youtube"></i></a>
...
def run(name):
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        main_file = None
        # Step 1: Default core files checking
        for f in ["main.py", "bot.py", "app.py", "index.js", "server.js"]:
            if os.path.exists(os.path.join(extract_dir, f)):
                main_file = f
                break
        
        # Step 2: Auto file finder scanner (Jaise Tghosting.py ya koi custom script)
