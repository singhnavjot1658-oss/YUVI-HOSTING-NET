...
def run(name):
    ...
    py_files = [f for f in all_files if f.endswith('.py') and os.path.isfile(os.path.join(extract_dir, f))]
    js_files = [f for f in all_files if f.endswith('.js') and os.path.isfile(os.path.join(extract_dir, f))]
    ...
    if not main_file:
        with open(log_path, "a", encoding="utf-8") as log_file:
            log_file.write("\n[SYSTEM ERROR] No executable .py or .js script found!\n")
        return redirect(url_for("index"))

    req_path = os.path.join(extract_dir, "requirements.txt")
    
    # CRITICAL: STRICT MANDATORY REQUIREMENTS.TXT CHECK FOR PYTHON
    if is_python and not os.path.exists(req_path):
        with open(log_path, "w", encoding="utf-8") as log_file:
            log_file.write("\n[SYSTEM ERROR] Please add requirements.txt to run your bot!\n")
        return redirect(url_for("index"))

    with open(log_path, "a", encoding="utf-8") as log_file:
        if is_python:
            log_file.write(f"\n[SYSTEM] Auto-Detected Python Script: '{main_file}'\n")
            log_file.write("[SYSTEM] Pre-installing requirements.txt packages...\n")
            log_file.flush()
            subprocess.run([sys.executable, "-m", "pip", "install", "-r", req_path], stdout=log_file, stderr=log_file)
            cmd = [sys.executable, main_file]
        else:
            log_file.write(f"\n[SYSTEM] Auto-Detected Node.js Script: '{main_file}'\n")
            pkg_path = os.path.join(extract_dir, "package.json")
            if os.path.exists(pkg_path):
                log_file.write("[SYSTEM] Installing npm packages...\n")
                log_file.flush()
...
def get_log(name):
    ...
    # --- ULTRA FAST LAZY KILLER: AUTO RESTART ON MODULE NOT FOUND ---
    if p and p.poll() is not None:
        if "ModuleNotFoundError: No module named" in log_content:
            try:
                # Find the module name from logs
                last_line = [line for line in log_content.split('\n') if "ModuleNotFoundError: No module named" in line][-1]
                mod_name = last_line.split("No module named")[-1].replace("'", "").replace('"', '').strip()
                
                # Clean up specific alias mappings
                package_mapping = {
                ...
                install_name = package_mapping.get(mod_name, mod_name)
                
                req_path = os.path.join(extract_dir, "requirements.txt")
                if os.path.exists(req_path):
                    # Check if already added to prevent infinite loop loops
                    with open(req_path, "r", encoding="utf-8") as rf:
                        req_lines = rf.read().splitlines()
                    
                    if install_name not in req_lines:
                        with open(req_path, "a", encoding="utf-8") as wf:
                            wf.write(f"\n{install_name}")
                        
                        with open(log_path, "a", encoding="utf-8") as log_file:
                            log_file.write(f"\n[SYSTEM ALERT] Missing module detected: '{mod_name}'. Auto-added '{install_name}' to requirements.txt and restarting...\n")
                        
                        # Trigger automated seamless crash boot recovery protocol immediately
                        ...
                        # Redirect inside local process context context structure safely 
                        subprocess.Popen([sys.executable, os.path.abspath(__file__)], env=os.environ)
                        # Let Flask route context run standalone initialization fallback
                        # Simply simulate clean background restart sequence trigger safely
                        # We trigger dynamic fast execution block manually:
                        # It will evaluate inside frontend polling seamlessly.
            except Exception as e:
                pass
        
        if f"{user_name}_{name}" in db_data.get("start_times", {}):
            is_running = False
            del db_data["start_times"][f"{user_name}_{name}"]
            save_db(db_data)

    return jsonify({
        "log": log_content[-3000:] if log_content else "> System ready. Press START to activate server context.", 
