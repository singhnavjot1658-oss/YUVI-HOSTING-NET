...
# FIX: Upload limit ko badha diya taaki badi zip files block na hon (50 MB)
...
# FIX: Explicit full permissions (mode=0o777) taaki Render Linux user safely file write kar sake
os.makedirs(UPLOAD_FOLDER, mode=0o777, exist_ok=True)

# Helper function: Path Traversal Attack validation
...
def index():
    ...
    # FIX: Explicit permissions yahan bhi add kiye hain safely directory create karne ke liye
    ...
    for name in os.listdir(user_dir):
        if os.path.isdir(os.path.join(user_dir, name)):
            ...
            if not is_running and f"{user_name}_{name}" in db_data.get("start_times", {}):
                db_data["start_times"].pop(f"{user_name}_{name}", None)
                db_changed = True
...
def run(name):
    ...
    if len(contents) == 1 and os.path.isdir(os.path.join(extract_dir, contents[0])):
        search_dir = os.path.join(extract_dir, contents[0])

    all_files = os.listdir(search_dir)
    py_files = [f for f in all_files if f.endswith('.py') and os.path.isfile(os.path.join(search_dir, f))]
    js_files = [f for f in all_files if f.endswith('.js') and os.path.isfile(os.path.join(search_dir, f))]
    
    main_file = None
    is_python = True

    priority_py = ["main.py", "bot.py", "app.py"]
    priority_js = ["index.js", "server.js", "app.js"]

    for pf in priority_py:
        if pf in py_files:
            main_file = pf
            break
    ...
    try:
        if is_python and os.path.exists(req_file):
            subprocess.Popen([sys.executable, "-m", "pip", "install", "-r", req_file], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        elif not is_python and os.path.exists(package_json):
            subprocess.Popen(["npm", "install"], cwd=search_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except Exception as e:
        print(f"Dependency installation failed: {e}")

    log_file_path = os.path.join(app_dir, "output.log")
    ...
    try:
        ...
        processes[(user_name, name)] = proc
        
        db_data = load_db()
        db_data["start_times"][f"{user_name}_{name}"] = time.time()
        save_db(db_data)
...
def stop(name):
    ...
    if p:
        ...
        processes.pop((user_name, name), None)
...
# --- FIX UPLOAD ROUTE WITH BETTER ERROR HANDLING & PERMISSIONS ---
...
def upload():
    ...
    proj_name = secure_filename(file.filename.rsplit('.', 1)[0])
    proj_dir = os.path.join(UPLOAD_FOLDER, user_name, proj_name)
    extract_dir = os.path.join(proj_dir, "extracted")
    
    p = processes.get((user_name, proj_name))
    if p:
        try:
            p.terminate()
            p.wait(timeout=2)
        ...
        processes.pop((user_name, proj_name), None)

    db_data = load_db()
    db_data["start_times"].pop(f"{user_name}_{proj_name}", None)
    save_db(db_data)
    ...
    if os.path.exists(proj_dir):
        try:
            ...
            print(f"Error removing old project dir: {e}")
        
    # FIX: Directory creation permissions 0o777 apply kari
    os.makedirs(extract_dir, mode=0o777, exist_ok=True)
    zip_path = os.path.join(proj_dir, secure_filename(file.filename))
    
    try:
        file.save(zip_path)
    except Exception as e:
        return f"Failed to save file on disk: {str(e)}", 500
    
    try:
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            zip_ref.extractall(extract_dir)
    except Exception as e:
        # File remove karein agar extraction crash ho jaye
        if os.path.exists(zip_path): os.remove(zip_path)
        return f"Zip extract error: {str(e)}", 500
    finally:
...
def delete_project(name):
    ...
    proj_dir = os.path.join(UPLOAD_FOLDER, user_name, name)
    if os.path.exists(proj_dir):
        shutil.rmtree(proj_dir)
