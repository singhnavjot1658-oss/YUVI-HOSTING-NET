...
<script>
    ...
    function deleteFile(f) { Swal.fire({ title: 'मुছে ফেলবেন?', text: `${f} স্থায়ীভাবে ডিলিট হবে!`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ff4d4d', background: '#0a0a0a', color: '#fff' }).then(res => { if(res.isConfirmed) { fetch('/delete_file', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({project: activeProj, filename: f}) }).then(() => refreshFiles()); } }); }
...
# --- FIX: ADDED CRITICAL MISSING ROUTES FOR FULL FUNCTIONALITY ---
