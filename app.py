...
# template_folder="." lagane se Flask direct main folder se HTML files uthayega
...
# --- SAFE FIREBASE ADMIN SETUP FOR PUBLIC GITHUB (FIXED) ---
if not firebase_admin._apps:
    # Render ke Environment Variable se credentials uthayega
    ...
    if firebase_creds_json:
        try:
            ...
            # Dynamic bucket name configuration
            project_id = creds_dict.get("project_id", "yuvi-hosting-net")
            bucket_name = f"{project_id}.firebasestorage.app" if ".firebasestorage.app" in project_id else f"{project_id}.appspot.com"
            
            firebase_admin.initialize_app(cred, {
                "databaseURL": "https://yuvi-hosting-net-default-rtdb.firebaseio.com/",
                "storageBucket": bucket_name
            })
            print("Firebase Admin successfully initialized via Environment Variable!")
        except Exception as e:
            print(f"Firebase Initialization Error: {e}")
    else:
        print("ERROR: FIREBASE_CREDS Environment Variable nahi mila! Render dashboard check karein.")

# --- DB FUNCTIONS (OFFICIAL FIREBASE ADMIN) ---
def load_db():
    try:
        ref = db.reference("system_data")
        data = ref.get()
        if not data:
            data = {"user_pw": "codex123", "users": {}, "start_times": {}}
            ref.set(data)
...
def login():
    if request.method == "POST":
        ...
        if l_type == "admin":
            ...
            if username and username not in db_data["users"]:
                db_data["users"][username] = db_data["user_pw"]
                save_db(db_data)
            if username and pw == db_data["users"].get(username):
...
def stop(name):
    ...
    db_data = load_db()
    if f"{user_name}_{name}" in db_data["start_times"]:
