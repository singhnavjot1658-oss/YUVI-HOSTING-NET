...
def load_db():
    ...
    # Agar database bilkul khali (null) hai, tabhi default set karein
    if data is None:
        default_data = {"user_pw": "codex123", "users": {}, "start_times": {}}
        ref.set(default_data)
        return default_data
        
    # Sirf missing keys ko fill karein, poora data overwrite na karein
...
def login():
    if request.method == "POST":
        ...
        if l_type == "admin":
            ...
            if username:
                # 🛑 Firebase se directly single user-node fetch karenge taaki data reset na ho
                user_existing_pw = ref.child("users").child(username).get()
                
                if user_existing_pw is None:
                    # Agar user naya hai, tabhi global default password assign hoga
