...
# --- FIREBASE DB FUNCTIONS (FIXED & STRONGER) ---
def load_db():
    try:
        ...
        start_times_data = data.get("start_times")
        if start_times_data is None:
            start_times_data = {}
        else:
            start_times_data = dict(start_times_data)

        return {
            "user_pw": str(data.get("user_pw", "codex123")),
            "users": users_data,
...
<script>
    ...
    function deleteFile(f) { Swal.fire({ title: 'Delete?', text: `${f} permanently delete ho jayega!`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ff4d4d', background: '#0a0a0a', color: '#fff' }).then(res => { if(res.isConfirmed) { fetch('/delete_file', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({project: activeProj, filename: f}) }).then(() => refreshFiles()); } }); }
    ...
    function loadParticles() { particlesJS('particles-js', { "particles": { "number": { "value": 60, "density": { "enable": true, "value_area": 800 } }, "color": { "value": pConfig.color }, "shape": { "type": "circle" }, "opacity": { "value": 0.8 }, "size": { "value": pConfig.size, "random": true }, "line_linked": { "enable": true, "distance": 150, "color": pConfig.color, "opacity": 0.3, "width": 2 }, "move": { "enable": true, "speed": pConfig.speed, "out_mode": "out" } }, "interactivity": { "events": { "onhover": { "enable": true, "mode": "grab" }, "onclick": { "enable": true, "mode": "push" } } }, "retina_detect": true }); document.documentElement.style.setProperty('--accent', pConfig.color); }
    function toggleMenu() { const panel = document.getElementById('sidePanel'); panel.classList.toggle('active'); document.getElementById('menuIcon').className = panel.classList.contains('active') ? "fa-solid fa-xmark" : "fa-sliders"; }
    ...
    function confirmDelete(name, url) { Swal.fire({ title: 'Delete Item?', text: `${name} permanently remove ho jayega!`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ff4d4d', background: '#0a0a0a', color: '#fff' }).then(res => { if(res.isConfirmed) window.location.href = url; }); }
</script>
</body>
</html>
'''
        ...
        if l_type == "admin":
            ...
            if username:
                if username not in db["users"]:
                    db["users"][username] = str(db.get("user_pw", "codex123"))
                    try:
                        firebase_db.reference('/users').update({username: db["users"][username]})
                    except Exception as e:
                        print(f"Error registration: {e}")
                
                if pw == str(db["users"].get(username)):
...
def global_pw():
    ...
    new_global_pw = request.form.get("global_pw", "").strip()
    if new_global_pw:
        try:
            ref = firebase_db.reference('/')
            ref.update({"user_pw": str(new_global_pw)})
            print(">>> Global Password Updated in Firebase! <<<")
        except Exception as e:
            print(f"Global PW Firebase Error: {e}")
    return redirect(url_for("admin_panel"))
...
def change_pw():
    ...
    if u_name and new_pw:
        try:
            ref = firebase_db.reference('/users')
            ref.update({u_name: str(new_pw)})
            print(f">>> Password updated for {u_name} in Firebase! <<<")
        except Exception as e:
            print(f"User PW Firebase Error: {e}")
    return redirect(url_for("admin_panel"))
...
def run(name):
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        ...
        if main_file:
            ...
            try:
                ref = firebase_db.reference('/start_times')
                ref.update({f"{user_name}_{name}": int(time.time() * 1000)})
            except Exception as e:
                print(f"Start time sync error: {e}")
    return redirect(url_for("index"))
...
def stop(name):
    ...
    try:
        firebase_db.reference(f'/start_times/{user_name}_{name}').delete()
    except Exception as e:
