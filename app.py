...
# --- REAL-TIME AUTO TERMINATE MIDDLEWARE ---
@app.before_request
def check_user_plan_status():
    ...
    if 'username' in session and not session.get('is_admin'):
        ...
        if user_info.get("plan_type") != "permanent":
            ...
            is_expired = user_info.get("status") == "expired"
...
def index():
    ...
    # FIXED: Load UI mode correctly from DB
...
# FIXED: UI Mode Persistence Bug Handled Safely
...
def save_theme():
    ...
    # Preserve current mode if not explicitly updated
