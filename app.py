...
# --- DB STRUCTURE WITH API KEY SUPPORT ---
def load_db():
    with db_lock:
        if not os.path.exists(DB_FILE):
            default = {
                ...
                "start_times": {}, 
                "themes": {}, 
                "admin_settings": {"api_key": generate_random_api_key()}
            }
        ...
        with open(DB_FILE, "r") as f:
            try:
                ...
                if "admin_settings" not in data: 
                    data["admin_settings"] = {"api_key": generate_random_api_key()}
                return data
            except:
                return {
                    "users": {}, 
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        body { background: var(--bg); color: white; font-family: sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; padding: 15px; }
        .login-card { background: #090d16; padding: 30px; border-radius: 20px; width: 100%; max-width: 380px; text-align: center; border: 1px solid rgba(0, 255, 255, 0.2); box-shadow: 0 0 25px rgba(0, 255, 255, 0.1); }
        .lock-container { width: 65px; height: 65px; background: rgba(0, 255, 255, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px; border: 1px solid var(--primary); }
        ...
        input, select { width: 100%; padding: 10px; margin: 8px 0; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); color: #fff; font-size: 13px; outline: none; }
        button { width: 100%; padding: 11px; border-radius: 8px; border: none; background: linear-gradient(45deg, var(--sec), var(--primary)); color: #fff; font-weight: bold; cursor: pointer; margin-top: 15px; text-transform: uppercase; font-size: 13px; }
...
# --- MODERN ADMIN HTML WITH TAB NAVIGATION ---
ADMIN_HTML = '''
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>JUBARAJ Panel | Compact Dashboard</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
        :root { --bg: #06090e; --card: #0d1117; --accent: #00ffff; --sec: #7000ff; --text: #c9d1d9; --border: rgba(255,255,255,0.08); }
        body { background: var(--bg); color: var(--text); padding-bottom: 70px; }
        
        .header { display: flex; justify-content: space-between; align-items: center; padding: 12px 15px; background: #0d1117; border-bottom: 1px solid var(--border); sticky: top:0; z-index: 100;}
        ...
        input, select, textarea { width: 100%; padding: 8px 10px; border-radius: 6px; border: 1px solid #21262d; background: #010409; color: white; font-size: 12px; outline: none; }
        ...
        .btn-primary { background: linear-gradient(45deg, var(--sec), var(--accent)); color: #000; }
        ...
        .navbar { position: fixed; bottom: 0; left: 0; right: 0; background: #0d1117; display: flex; justify-content: space-around; padding: 8px 0; border-top: 1px solid var(--border); z-index: 1000; }
        ...
        .api-box { background: #000; padding: 10px; border-radius: 6px; border: 1px dashed var(--accent); font-family: monospace; font-size: 11px; word-break: break-all; margin: 8px 0; color: #39d353;}
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="container">
            ...
            <div class="card">
                ...
                <div class="user-item">
                    ...
                    <div style="display:flex; gap:6px; margin-top:6px;">
                        ...
                        <a href="/admin/delete_user/{{ u_name }}" class="btn-sm btn-danger" style="padding: 3px 8px;" onclick="return confirm('Delete user data?')">Delete</a>
                    </div>
                </div>
                {% endfor %}
            </div>
        ...
        <div id="tab-api" class="tab-content">
            <div class="card">
                ...
                <div class="api-box">{{ api_key }}</div>
                
                <form action="/admin/regen_key" method="post" style="margin-top: 8px;">
    ...
    <div class="navbar">
        <div class="nav-item active" onclick="switchTab('tab-users', this)">
...
def admin_panel():
    ...
    db = load_db()
    api_key = db.get("admin_settings", {}).get("api_key", "")
    return render_template_string(ADMIN_HTML, users=db["users"], api_key=api_key)
