...
try:
    ...
    firebase_admin.initialize_app(cred, {
        'databaseURL': "https://yuvi-hosting-default-rtdb.firebaseio.com/"  # Fixed: Added missing string quotes
    })
    ...
    if request.method == "POST":
        ...
        if l_type == "admin":
            if username == ADMIN_USER and pw == ADMIN_PASS:
                ...
                return redirect(url_for("login"))  # Incorrect Admin Details
        else:
            # Fixed: Standard users route now uses independent logic block
            if username == ADMIN_USER:
                return redirect(url_for("login")) # Prevent Admin tracking as general user
                
            if username and username not in db_data["users"]:
                # Default system user_pw (yuvi123) will be applied
