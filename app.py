...
def run(name):
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        ...
        if os.path.exists(extract_dir):
            ...
            # 1. पहले प्राथमिकता वाली फाइलें चेक करें (ताकि standard नाम पहले रन हों)
            ...
            # 2. अगर standard नाम नहीं हैं, तो फोल्डर में जो भी पहली .py या .js फाइल मिले उसे चुनें
            if not main_file:
                for f in all_files:
                    if f.endswith('.py') or f.endswith('.js'):
                        main_file = f
                        break  # पहली मिलने वाली स्क्रिप्ट को ही मुख्य फाइल मान लें
