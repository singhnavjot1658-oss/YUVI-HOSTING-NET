...
def index():
    ...
    for name in os.listdir(user_dir):
        if os.path.isdir(os.path.join(user_dir, name)):
            ...
            # BUG FIX 2: Check accurate state, don't trust static memory dict if container restarted
            is_running = (p and p.poll() is None)
            
            # Sync Firebase DB if process died during background restart
...
def run(name):
    ...
    # Extra Safe Handling: Agar folder ke andar single parent nested folder ban gaya ho zip extract se
    ...
    try:
        # BUG FIX 3: Removed `.wait()` so main Flask thread never blocks/freezes during deployment
...
def upload():
    ...
    # BUG FIX 1: Jab naya zip aaye, toh pehle purane chalte hue process ko terminate karo aur database clear karo
...
def delete_project(name):
    ...
    # Properly call internal stop routine
    p = processes.get((user_name, name))
    if p:
        try:
            p.terminate()
