...
app = Flask(__name__)
app.secret_key = "YUVRAJ_HOSTING_PRO"
...
# Apni Firebase Realtime Database URL yahan dalein
...
def load_db():
    # Pehle local structure build/read karte hain
    ...
    # Firebase se data sync karne ki koshish karte hain
    try:
        ref = db.reference("hosting_db")
        firebase_data = ref.get()
        if firebase_data:
            # Agar Firebase me data hai, toh use local se merge/override karte hain
...
def save_db(data):
    ...
    # Firebase Cloud Cloud backup save
    try:
        ref = db.reference("hosting_db")
        ref.set(data)
    except Exception as e:
...
<body>
    ...
    <div class="header"><h2><i class="fa-solid fa-shield-halved"></i> WELCOME {{ session['username'] }} (OWNER)</h2><a href="/logout" class="btn btn-logout"><i class="fa-solid fa-power-off"></i></a></div>
