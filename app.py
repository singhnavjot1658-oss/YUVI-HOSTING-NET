...
def run(name):
    ...
    # --- ENHANCED DYNAMIC DETECTOR ---
    # Extract folder ki saari files read karo
    all_files = os.listdir(extract_dir)
    
    # Target elements verify karo (.py and .js matching targets only)
    ...
    # Priority checklists setup
    priority_py = ["main.py", "bot.py", "app.py"]
    priority_js = ["index.js", "server.js", "app.js"]

    # 1. Pehle priority file checks karo (Python)
    for pf in priority_py:
        if pf in py_files:
            main_file = pf
            break
            
    # 2. Agar koi priority file nahi mili, toh pehli generic .py file select karo
    if not main_file and py_files:
        main_file = py_files[0]
        
    # 3. Agar python missing hai, toh JS fields evaluate karo
    if not main_file:
    ...
    # 4. Agar priority JS nahi hai, toh pehli generic .js file select karo
    if not main_file and js_files:
        main_file = js_files[0]
        is_python = False

    # Agar koi execute hone layak script mili hi nahi, toh fallback structure use karo
    if not main_file:
        log_path = os.path.join(app_dir, "logs.txt")
        with open(log_path, "a", encoding="utf-8") as log_file:
            log_file.write("\n[SYSTEM ERROR] No executable .py or .js script found in root directory!\n")
        return redirect(url_for("index"))

    log_path = os.path.join(app_dir, "logs.txt")
    
    # Safely write execution dynamic context down to logs
    with open(log_path, "a", encoding="utf-8") as log_file:
        if is_python:
            log_file.write(f"\n[SYSTEM] Auto-Detected Python Target Script: '{main_file}'\n")
            req_path = os.path.join(extract_dir, "requirements.txt")
            if os.path.exists(req_path):
                log_file.write("[SYSTEM] Installing pip packages from requirements.txt...\n")
                log_file.flush()
                subprocess.run([sys.executable, "-m", "pip", "install", "-r", req_path], stdout=log_file, stderr=log_file)
            else:
                log_file.write("[SYSTEM] requirements.txt missing! Scanning code modules for auto-installer...\n")
                log_file.flush()
                try:
                    package_mapping = {
            ...
            log_file.write(f"\n[SYSTEM] Auto-Detected Node.js Target Script: '{main_file}'\n")
    ...
    # Open standard out descriptor pointer handles safely
