    ...
    while True:
        try:
            ...
            for username, info in list(db["users"].items()):
                ...
                if expiry_str:
                    expiry_ts = datetime.strptime(expiry_str, "%Y-%m-%dT%H:%M").timestamp()
                    if now > expiry_ts and info.get("status") != "expired":
                        db["users"][username]["status"] = "expired"
                        changed = True
                        # Auto stop background processes instantly
                        stop_user_apps(username)
                        print(f"[PAID SYSTEM] Access expired for user: {username}. All apps stopped.")
            if changed:
                save_db(db)
        except Exception as e:
            print(f"Enforcement Loop Error: {e}")
        time.sleep(30) # Har 30 seconds me check karega
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        body { background: var(--bg); color: white; font-family: 'Orbitron', sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; overflow: hidden; padding: 10px; }
        #particles-js { position: fixed; width: 100%; height: 100%; z-index: 1; }
        .login-card { position: relative; z-index: 10; background: var(--glass); padding: 35px 25px; border-radius: 25px; width: 100%; max-width: 340px; text-align: center; border: 1px solid rgba(0, 255, 255, 0.15); backdrop-filter: blur(20px); box-shadow: 0 25px 45px rgba(0,0,0,0.5); transition: 0.4s; }
        .lock-container { width: 80px; height: 80px; background: rgba(0, 255, 255, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; border: 2px solid var(--primary); box-shadow: 0 0 20px var(--primary); }
        ...
        input, select { width: 100%; padding: 14px; margin: 10px 0; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); color: #fff; outline: none; font-size: 14px; }
        button { width: 100%; padding: 15px; border-radius: 12px; border: none; background: linear-gradient(45deg, var(--sec), var(--primary)); color: #fff; font-weight: bold; cursor: pointer; margin-top: 15px; text-transform: uppercase; letter-spacing: 2px; }
        .error-msg { color: #ff4757; font-size: 13px; margin-top: 10px; display: block; }
    </style>
</head>
<body>
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body { background: var(--bg); color: var(--text); font-family: sans-serif; padding: 15px; overflow-x: hidden; }
        ...
        .card { background: var(--card); padding: 15px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.08); margin-bottom: 20px; width: 100%; overflow: hidden; }
        ...
        input, select, textarea { width: 100%; padding: 12px; border-radius: 10px; border: 1px solid #333; background: rgba(0,0,0,0.3); color: white; display: block; outline: none; }
        ...
        .user-item { background: rgba(255,255,255,0.03); border-radius: 12px; padding: 15px; margin-bottom: 10px; border: 1px solid rgba(255,255,255,0.05); width: 100%; }
    ...
    <div class="grid">
        <div class="card">
            ...
            <form action="/admin/create_user" method="post" class="input-group">
                ...
                <label style="font-size:12px; color: var(--accent);">Access Valid Till (Date & Time):</label>
    ...
    <div class="card">
        ...
        <div class="user-item">
            ...
            <p style="font-size:12px; margin: 8px 0; opacity: 0.8;">
                <i class="fa-solid fa-clock"></i> Expiry Date: <b>{{ u_info.expiry.replace('T', ' ') }}</b>
            </p>
...
def get_alert():
    if 'username' not in session: return jsonify({"message": ""})
    user_name = session['username']
    db = load_db()
    msg = db["users"].get(user_name, {}).get("broadcast", "")
...
def login():
    ...
    if request.method == "POST":
        ...
        if l_type == "admin":
            ...
            # Open sign-up blocks. Only Admin injected entries read here
...
def index():
    ...
    # Extra Security Check for active login session
    db = load_db()
    if not session.get('is_admin') and user_name in db["users"]:
        if db["users"][user_name]["status"] == "expired":
            session.clear()
    ...
    # Injecting safe custom runtime view element for broadcast directly inside index layout
...
# --- NEW ADMIN HANDLERS (PAID MODULES) ---
...
def create_user():
    ...
    expiry = request.form.get("expiry") # format: YY-MM-DDTHH:MM
    
    if username and password and expiry:
        db = load_db()
        db["users"][username] = {
            "password": password,
            "expiry": expiry,
...
def delete_user(username):
    ...
    if username in db["users"]:
        ...
        stop_user_apps(username)
        # Delete data structure entries
        del db["users"][username]
        if username in db.get("themes", {}): del db["themes"][username]
        # Physically clear files
...
def broadcast():
    ...
    if target == "ALL_USERS":
        for u in db["users"]:
            db["users"][u]["broadcast"] = msg
    elif target in db["users"]:
        db["users"][target]["broadcast"] = msg
        
    save_db(db)
    return redirect(url_for("admin_panel"))

# --- KEEPING EXISTING FILE MANAGER LOGICS PRESERVED ---
...
def run(name):
    user_name = session['username']
    db = load_db()
    if user_name in db["users"] and db["users"][user_name]["status"] == "expired":
        return redirect(url_for("index"))
