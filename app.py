    ...
    while True:
        try:
            ...
            for username, info in list(db["users"].items()):
                ...
                if expiry_str:
                    try:
                        # Parsing compatibility logic for input string formats
                        if "T" in expiry_str:
                    ...
                    # FIX: Yahan update aur condition ensure ki hai
                    if now > expiry_ts:
                        print(f"[SYSTEM] User {username} completely terminated due to expiry.")
                        db["users"][username]["status"] = "expired"
                        save_db(db) # Local database block lock safe state update
                        stop_and_clean_user(username)
                        changed = True
            if changed:
                db = load_db() # Reload update state array
        except Exception as e:
