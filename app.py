...
def run(name):
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        ...
        if main_file:
            ...
            # --- AUTO DEPENDENCY SMART PACKAGE INSTALLER ---
            if main_file.endswith('.py'):
                req_path = os.path.join(extract_dir, "requirements.txt")
                if os.path.exists(req_path):
                    log_file.write("\n[SYSTEM] Installing pip packages from requirements.txt...\n")
                    log_file.flush()
                    subprocess.run(["pip", "install", "-r", req_path], stdout=log_file, stderr=log_file)
                else:
                    # Smart scanner for scripts lacking requirements.txt
                        ...
                        for line in lines:
                            if line.startswith("import ") or "import" in line:
                                ...
                                if "import" in parts:
                                    ...
                                    if idx + 1 < len(parts):
                                        mod = parts[idx + 1].split(',')[0].split('.')[0].strip()
                                        if mod not in ["os", "sys", "time", "json", "random", "math", "re", "requests", "subprocess", "io", "shutil"]:
                                            subprocess.run(["pip", "install", mod], stdout=log_file, stderr=log_file)
                    except Exception as ex:
                        print(f"Auto-import-installer exception: {ex}")
