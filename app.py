...
app = Flask(__name__)
app.secret_key = "YUVI_CODEX_HOSTING_PRO"
...
<script>
    ...
    function deleteFile(f) { Swal.fire({ title: 'মুছে ফেলবেন?', text: `${f} স্থায়ীভাবে ডিলিট होगा!`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ff4d4d', background: '#0a0a0a', color: '#fff' }).then(res => { if(res.isConfirmed) { fetch('/delete_file', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({project: activeProj, filename: f}) }).then(() => refreshFiles()); } }); }
    ...
    if request.method == "POST":
        ...
        if l_type == "admin":
            if username == "JUBARAJ" and pw == ADMIN_PASS:
                session['is_admin'], session['username'] = True, "JUBARAJ"
