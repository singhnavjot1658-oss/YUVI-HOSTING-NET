...
# Agar Render pe /data disk attached hai toh wahan save hoga, nahi toh local pe fallback
...
def run(name):
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        ...
        if main_file:
            ...
            # --- AUTO DEPENDENCY PACKAGE INSTALLER ---
...
def delete(name):
    ...
    if os.path.exists(app_dir) and is_safe_path(os.path.join(UPLOAD_FOLDER, user_name), app_dir): 
