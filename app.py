...
# --- HELPER FUNCTION TO CHECK REALTIME EXPIRY ---
def check_is_expired(user_info):
    """Realtime check karta hai bina background thread ke wait kiye"""
    ...
    try:
        ...
        # Realtime strict epoch timestamp validation
...
def stop_and_clean_user(user_name):
    """User ke chal rahe apps ko hardcore tree-kill karta hai aur hosted data reset karta hai."""
    db = load_db()
    
    # 1. HARD-KILL PROCESS TREE INSIDE OS ENVIRONMENT
    ...
    # 2. Local uploads data directory clear
    ...
    while True:
        try:
            ...
            for username, info in list(db["users"].items()):
                ...
                # Strict dynamic runtime enforcement
                if check_is_expired(info):
                    if info.get("status") != "expired":
                        db["users"][username]["status"] = "expired"
                        changed = True
                        stop_and_clean_user(username)
                        print(f"[AUTO-EXPIRY] Wiped and locked user: {username}")
            if changed:
                save_db(db)
        except Exception as e:
            print(f"Enforcement Loop Error: {e}")
        time.sleep(2) # Engine checks down to 2 seconds
...
# --- REALTIME API & SECURITY GUARD SYSTEM ---
...
def get_alert():
    ...
    # SYSTEM INTERCEPTOR GUARD: Dynamic page-refresh kicking mechanics
    if not session.get('is_admin') and user_name in db["users"]:
        if check_is_expired(db["users"][user_name]):
            # Auto-terminate triggers straight away from the core background state sync logic
            if db["users"][user_name].get("status") != "expired":
                db["users"][user_name]["status"] = "expired"
                save_db(db)
                stop_and_clean_user(user_name)
            return jsonify({"message": "", "id": "", "expired_kick": True})
        ...
        if l_type == "admin":
            ...
            if username in db["users"]:
                ...
                if user_data["password"] == pw:
                    # BLOCK AT THE GATE: Background loop ka bina wait kiye direct real-time validation
                    if check_is_expired(user_data):
                        # Force update database on login breach attempt
                        db["users"][username]["status"] = "expired"
                        save_db(db)
                        stop_and_clean_user(username)
...
def index():
    ...
    if db["users"].get(user_name, {}).get("plan_type") == "permanent":
        expiry_display = "Lifetime Permanent"
    else:
        expiry_display = db["users"].get(user_name, {}).get("expiry", "N/A").replace("T", " ")
...
def list_files(name):
    ...
    user_name = session['username']
    db = load_db()
    if user_name in db["users"] and check_is_expired(db["users"][user_name]):
