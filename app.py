...
try:
    # Render par agar local file mil jaye to thik, warna absolute fallback lagaya hai
...
def index():
    ...
    for name in os.listdir(user_dir):
        if os.path.isdir(os.path.join(user_dir, name)):
            ...
            # Fallback tracking if Gunicorn creates multi-workers
            is_running = (p and p.poll() is None) or f"{user_name}_{name}" in db_data["start_times"]
            apps_list.append({"name": name, "running": is_running})
            
    # Render safety fallback agar templates/index.html na mile
    try:
