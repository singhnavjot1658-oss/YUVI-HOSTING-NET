...
# --- FIREBASE DB FUNCTIONS (FIXED ALTERNATIVE) ---
def load_db():
    try:
...
def login():
    if request.method == "POST":
        ...
        if l_type == "admin":
            ...
            if username:
                # Agar user database me nahi hai, toh use naya password allocate karo
                if username not in db["users"]:
                    db["users"][username] = str(db.get("user_pw", "codex123"))
                    save_db(db)
                
                # Sahi authentication match hone par log-in logic
...
def login_as(username):
    if not session.get('is_admin') or session.get('username') != "JUBARAJ": return redirect(url_for("login"))
    session['username'] = username
    session['is_admin'] = False # User interface access ke liye set kiya
    return redirect(url_for("index"))
