...
try:
    ...
    if firebase_json:
        # Render Environment Variable se credentials uthane ke liye
        cred_dict = json.loads(firebase_json)
        cred = credentials.Certificate(cred_dict)
    else:
        # Local machine fallback file ke liye
...
# --- FIREBASE DB FUNCTIONS (FIXED SAVE & LOAD) ---
def load_db():
    try:
...
def save_db(data):
    try:
        ref = firebase_db.reference('/')
        # .update() use karne se root structure crash nahi hota aur data efficiently save hota hai
        ref.update({
            "user_pw": data.get("user_pw", "codex123"),
            "users": data.get("users", {}),
            "start_times": data.get("start_times", {}),
            "user_themes": data.get("user_themes", {})
        })
...
<script>
    let startTimes = {}; 
    let pConfig = { color: "{{ saved_color }}", size: 38, speed: 4 };
...
def index():
    ...
    db = load_db()
    saved_color = db.get("user_themes", {}).get(user_name, "#00ffff")
    ...
    return render_template_string(INDEX_HTML, apps=apps_list, username=user_name, saved_color=saved_color)
