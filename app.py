...
# --- SAFE FIREBASE ADMIN SETUP (FIXED STORAGE BUCKET) ---
if not firebase_admin._apps:
    ...
    if firebase_creds_json:
        try:
            ...
            # Storage Bucket Fix: Sahi format auto-detect karne ke liye
            ...
            print(f"Firebase Status: Initialized successfully with bucket: {bucket_name}")
        except Exception as e:
            print(f"Firebase Initialization Error: {e}")
    else:
        print("ERROR: FIREBASE_CREDS Environment Variable nahi mila!")

# --- DB FUNCTIONS (FIXED REALTIME SYNC) ---
def load_db():
    try:
...
def save_db(data):
    try:
        ref = db.reference("system_data")
        ref.set(data)
        # Chhota sa delay taaki network requests proper complete ho sakein
        time.sleep(0.5) 
        return True
    except Exception as e:
...
def change_pw():
    ...
    if u_name in db_data["users"]:
        ...
        if save_db(db_data):
            time.sleep(0.5) # Force wait till updated
    return redirect(url_for("admin_panel"))
