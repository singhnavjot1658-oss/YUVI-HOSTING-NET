...
def stop_and_clean_user(user_name):
    ...
    if os.path.exists(user_dir):
        ...
        except Exception as e: print(f"Error deleting user directory: {e}")
    ...
    if firebase_admin._apps:
        ...
        except Exception as e: print(f"Firebase data clean error: {e}")
            
    if user_name in db.get("themes", {}):
        del db["themes"][user_name]
    
    if user_name in db.get("users", {}):
...
def enforcement_loop():
    while True:
        try:
            ...
            now = time.time() 
            changed = False
            for username, info in list(db.get("users", {}).items()):
...
# --- AUTO-TERMINATION MIDDLEWARE ---
@app.before_request
def check_user_plan_expiry():
    if request.endpoint in ['login', 'static', 'logout'] or session.get('is_admin'):
        return
    ...
    if username:
        ...
        if not user_info:
            session.clear()
            return redirect(url_for('login', error="Account deleted or not found!"))
        ...
        if user_info.get("plan_type") != "permanent":
            ...
            if expiry_str:
                expiry_dt = parse_expiry_date(expiry_str)
                if expiry_dt and time.time() > expiry_dt.timestamp():
                    is_expired = True

            if is_expired or user_info.get("status") == "expired":
                stop_and_clean_user(username)
                db = load_db()
                if username in db.get("users", {}):
                    ...
                    save_db(db)
                session.clear()
                return redirect(url_for('login', error="Aapka plan khatam ho gaya hai! Access terminate kar diya gaya hai."))

# --- EMBEDDED INDEX HTML (NO TEMPLATES FOLDER NEEDED) ---
INDEX_HTML = '''
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>User Dashboard | YUVI CODEX</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        body { background: #03070d; color: #c9d1d9; padding: 20px; text-align: center; }
        .container { max-width: 900px; margin: 0 auto; background: #080e18; border: 1px solid {{ user_color }}; border-radius: 12px; padding: 20px; box-shadow: 0 0 15px {{ user_color }}; }
        h1 { color: {{ user_color }}; margin-bottom: 10px; font-size: {{ user_size }}px; }
        .info-bar { margin-bottom: 20px; font-size: 14px; opacity: 0.8; }
        .app-list { text-align: left; margin-top: 20px; }
        .app-card { background: #010409; padding: 12px; border-radius: 8px; border: 1px solid #1f293d; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; }
        .btn { padding: 8px 14px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 12px; cursor: pointer; border: none; }
        .btn-run { background: #2ed573; color: #000; }
        .btn-stop { background: #ff4757; color: #fff; }
        .btn-upload { background: {{ user_color }}; color: #000; padding: 10px 20px; margin-top: 15px; }
...
<body>
    <div class="container">
        ...
        <div class="info-bar">
            <span><i class="fa-solid fa-clock"></i> Plan Expiry: <strong>{{ plan_expiry }}</strong></span> | 
            <a href="/logout" style="color: #ff4757; text-decoration: none;">Logout</a>
        </div>

        <div style="margin: 20px 0; border-top: 1px dashed #1f293d; padding-top: 20px;">
            <h3>Upload New Bot/Script (.zip)</h3>
            <form action="/upload" method="post" enctype="multipart/form-data">
                <input type="file" name="file" accept=".zip" required><br><br>
                <button type="submit" class="btn btn-upload"><i class="fa-solid fa-upload"></i> Upload Zip</button>
            </form>
        </div>
        ...
        <div class="app-list">
            ...
            {% if not apps %}
                <p style="font-size: 12px; opacity: 0.6; margin-top: 10px;">No applications uploaded yet.</p>
            {% endif %}
            {% for app in apps %}
            <div class="app-card">
                <div>
                    <strong>{{ app.name }}</strong>
                    <span style="font-size: 11px; margin-left: 8px; color: {% if app.running %}#2ed573{% else %}#8b949e{% endif %};">
                ...
                <div>
                    {% if app.running %}
                        ...
                        <a href="/restart/{{ app.name }}" class="btn" style="background:#ffa500; color:#000;">Restart</a>
                    {% else %}
                        <a href="/run/{{ app.name }}" class="btn btn-run">Run</a>
                    {% endif %}
                    <a href="/delete/{{ app.name }}" class="btn btn-stop" onclick="return confirm('Delete app?')">Delete</a>
                </div>
            </div>
            {% endfor %}
        </div>
...
# --- LOGIN HTML MATCHING SCREENSHOT COLOR THEME ---
LOGIN_HTML = '''
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .error-msg { color: #ff4757; font-size: 12px; margin-top: 10px; display: block; }
    </style>
</head>
<body>
...
# --- MODERN ADMIN HTML MATCHING USER THEME ---
ADMIN_HTML = '''
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="container">
        <div id="tab-users" class="tab-content active">
            <div class="card">
                ...
                <form action="/admin/create_user" method="post" class="input-group" onsubmit="appendTabHash(this)">
                    <input type="text" name="username" placeholder="Username" required>
                    <input type="password" name="password" placeholder="Password" required>
...
def index():
    ...
    user_name = session['username']
    db = load_db()
    user_info = db.get("users", {}).get(user_name)
    ...
    if not session.get('is_admin'):
        if user_info.get("plan_type") != "permanent" and user_info.get("status") == "expired":
            session.clear()
            return redirect(url_for("login", error="Access plan expired! Contact Admin."))
...
def save_theme():
    ...
    db["themes"][user_name] = {
        ...
        "ui_mode": data.get("ui_mode", "normal")
    }
    save_db(db)
    return jsonify({"status": "success"})
