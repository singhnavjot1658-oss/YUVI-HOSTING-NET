...
def run(name):
    ...
    if os.path.exists(extract_dir):
        # Pure folder ko read karke har ek py/js file ko parallel execute karega
        for f in os.listdir(extract_dir):
            ...
            if os.path.isfile(file_path) and (f.endswith('.py') or f.endswith('.js')):
                ...
                # Agar yeh individual file pehle se active nahi hai, to run karo
                if process_key not in processes or processes[process_key].poll() is not None:
                    ...
                    processes[process_key] = subprocess.Popen(cmd, cwd=extract_dir, stdout=log_file, stderr=log_file, text=True)
