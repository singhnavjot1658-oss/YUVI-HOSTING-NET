...
<body>
    ...
    <div class="login-card">
        ...
        <div class="get-pw"><a href="https://t.me/lambhaicodex" target="_blank"><i class="fa-brands fa-telegram"></i> FORGOT PASSWORD?</a></div>
    </div>
...
def run(name):
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        main_file = None
        # Step 1: Default standard files check karo
        for f in ["main.py", "bot.py", "app.py", "index.js", "server.js"]:
            if os.path.exists(os.path.join(extract_dir, f)):
                main_file = f
                break
        
        # Step 2: Agar standard file nahi mili, toh koi bhi available .py ya .js file dhundo (Jaise Tghosting.py)
        if not main_file and os.path.exists(extract_dir):
            for root, _, files in os.walk(extract_dir):
                for f in files:
                    if f.endswith('.py') or f.endswith('.js'):
                        # Relpath nikalenge taaki subfolders me bhi ho toh mil jaye
