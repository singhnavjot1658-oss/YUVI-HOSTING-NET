...
def enforcement_loop():
    while True:
        ...
        time.sleep(5)  # Heavily reduced overhead to avoid filesystem locking issues
            ...
            {% if users %}
                ...
                <div class="user-item">
                    ...
                    <p style="font-size:11px; margin: 8px 0; opacity: 0.7; display: flex; align-items: center; gap: 10px; font-family:sans-serif;">
                        ...
                        <span>Expiry: <b style="color:var(--neon-purple)">{{ u_info.expiry.replace('T', ' ') }}</b></span>
                        {% endif %}
                    </p>
...
# --- USER DASHBOARD HTML (BINA TEMPLATES FOLDER KE CHALNE KE LIYE) ---
INDEX_HTML = '''
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        body { background-color: #030508; color: white; font-family: 'Orbitron', sans-serif; padding: 20px; }
        .header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #00ffff; padding-bottom: 10px; margin-bottom: 20px; }
        .btn { padding: 10px 15px; border-radius: 5px; border: none; font-weight: bold; cursor: pointer; text-decoration: none; display: inline-block; color: white; background: #7000ff; }
        .btn-danger { background: #ff4757; }
        .app-card { background: #090d16; border: 1px solid rgba(0,255,255,0.2); border-radius: 10px; padding: 15px; margin-bottom: 15px; display: flex; justify-content: space-between; align-items: center; }
        .status-running { color: #34d399; font-weight: bold; }
...
<body>
    <div class="header">
        ...
        <div>
            <span>Plan Expiry: <b>{{ plan_expiry }}</b></span>
            <a href="/logout" class="btn btn-danger" style="margin-left: 15px;">Logout</a>
        </div>
    </div>

    <div style="margin-bottom: 30px; background: #090d16; padding: 20px; border-radius: 10px; border: 1px dashed #00ffff;">
        <h3>Upload New Project (.zip)</h3>
        <form action="/upload" method="post" enctype="multipart/form-data">
            <input type="file" name="file" accept=".zip" required style="margin-bottom: 10px; display: block;">
            <button type="submit" class="btn">Deploy App</button>
        </form>
    </div>

    <h3>Your Hosted Applications</h3>
    {% if apps %}
        {% for app in apps %}
        <div class="app-card">
            <div>
                ...
                <p style="margin: 5px 0 0 0; font-size: 12px;">
                    Status: {% if app.running %}<span class="status-running">RUNNING</span>{% else %}<span class="status-offline">OFFLINE</span>{% endif %}
                </p>
            </div>
            <div style="display: flex; gap: 10px;">
                {% if app.running %}
                    <a href="/stop/{{ app.name }}" class="btn btn-danger">Stop</a>
                    <a href="/restart/{{ app.name }}" class="btn">Restart</a>
                {% else %}
                    <a href="/run/{{ app.name }}" class="btn" style="background: #00ffff; color: black;">Start</a>
                {% endif %}
                <a href="/download/{{ app.name }}" class="btn" style="background: #2f3542;"><i class="fa-solid fa-download"></i></a>
                <a href="/delete/{{ app.name }}" class="btn btn-danger" onclick="return confirm('Wipe completely?')"><i class="fa-solid fa-trash"></i></a>
            </div>
        </div>
        {% endfor %}
    {% else %}
        <p style="opacity: 0.5;">No applications deployed yet.</p>
    {% endif %}
</body>
</html>
'''
...
def index():
    ...
    raw_expiry_str = user_info.get("expiry", "N/A")
    is_permanent = user_info.get("plan_type") == "permanent"
    
    if is_permanent:
        expiry_display = "Lifetime Permanent"
        js_expiry_iso = ""
    else:
        expiry_display = raw_expiry_str.replace("T", " ")
        js_expiry_iso = raw_expiry_str

    try:
        # BINA TEMPLATE FOLDER KE CHALANE KE LIYE RENDER_TEMPLATE_STRING USE KIYA HAI
        return render_template_string(INDEX_HTML, 
                                      apps=apps_list, 
                                      username=user_name,
                                      plan_expiry=expiry_display,
                                      js_expiry=js_expiry_iso,
                                      is_permanent=is_permanent,
                                      user_color=user_theme.get("color", "#00ffff"),
                                      user_size=user_theme.get("size", 38),
                                      user_speed=user_theme.get("speed", 4))
    except Exception as e:
        return f"<h3>Render Error:</h3><br><pre>{str(e)}</pre>"
...
if __name__ == "__main__":
    # Web server standard runtime allocation
